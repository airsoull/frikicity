<?PHP $this->pageTitle = Yii::app()->name." - Productos"; ?>
        	...
        	<ul>
        		...
        		<?PHP
        			...
        			foreach ($modelo as $m) {
                        $contar++;
        				?>
        				<li class="segment-<?PHP echo $contar; ?>"><a href="javascript:void(0)" data-value="<?PHP echo $m['id_modelo']; ?>"><?PHP echo $m['modelo'] ?></a></li>
        				<?PHP
        			}
        		?>
            </ul>
        ...
        <ul class="image-grid columns1" id="list">
            ...
            foreach ($model as $m) {
                ...
                <div class="columns four omega txt_box">
                    ...
                    <p><?PHP echo $m->descripcion; ?><br><br>
                        <?PHP 
                        $total = (($m->precio*$porcentaje->tarjeta)/100);
                        $total = $total+$m->precio;
                        ?>
                        Efectivo:&nbsp;<?PHP echo '$'.number_format($m->precio); ?><br>
                    Tarjeta de crédito:&nbsp;<?PHP echo '$'.number_format($total); ?>
                </p>
                </div>
            </li>
                <?PHP
